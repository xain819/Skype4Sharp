...
namespace Skype4Sharp.Events
{
    class Poller
    {
        ...
        {
            foreach (Thread runningPoll in runningPolls)
            {
                runningPoll.Abort();
            }
            runningPolls.Clear();
        ...
        public void StartPoll()
        {
            new Thread(() => // Main Poll
            {
                ...
                while (true)
                {
                    ...
                    {
                        if (!(thrownException.GetType().IsAssignableFrom(typeof(ThreadAbortException))))
                        {
                            Console.WriteLine(thrownException.ToString());
                            if (thrownException.ToString().Contains("404"))
                            {
                                parentSkype.Login(true);
                            }
                        }
                    }
                }
